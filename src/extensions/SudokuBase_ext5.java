...
public class SudokuBase_ext5 {
    ...
    // Extension 3.4: Implémentation de la construction de la grille complète de l'ordinateur
    public static int[][] genererGrilleCompleteOrdinateur(int nbSuccessions) {
    ...
    public static boolean peutEtreEvident(int[][] grille, int x, int y) {
        boolean[] valPossibles = new boolean[10];
        for(int n = 0; n <= 9; n++)
            valPossibles[n] = true;
        int nbValPoss = 9;
        ...
        for(int i = debCarre[0]; i < debCarre[0] + 3; i++) {
            for(int j = debCarre[1]; j < debCarre[1] + 3; j++) {
                int debChiffre = grille[i][j];
                if((i != x || j != y) && debChiffre != 0 && valPossibles[debChiffre]) {
                    valPossibles[debChiffre] = false;
                    nbValPoss--;
                }
            }
        }
        ...
        for(int n = 0; n < grille.length; n++) {
            ...
            // Ligne
            if(n != y && chiffreLigne != 0 && valPossibles[chiffreLigne]) {
                valPossibles[chiffreLigne] = false;
                nbValPoss--;
            }
            // Colonne
            if(n != x && chiffreColonne != 0 && valPossibles[chiffreColonne]) {
                valPossibles[chiffreColonne] = false;
                nbValPoss--;
            }
        }

        // On vérifie si c'est un trou évident
        return nbValPoss == 1;
    }
    ...
    public static boolean verifierNouvelEvident(int[][] grille, int x, int y) {
        if(peutEtreEvident(grille, x, y)) {
            ...
            // On retire dans la liste des possibilités toutes les valeurs des cases de la ligne et colonne courante
            for(int n = 0; n < grille.length; n++) {
                int chiffreLigne = grille[x][n];
                int chiffreColonne = grille[n][y];
